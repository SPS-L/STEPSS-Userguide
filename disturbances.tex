...
\section{\index{CONTINUE SOLVER}Continue Solver}
...
\begin{itemize}
	\item ALL: Update all injectors and network 
	\item NET: Update only network 
	\item ABL: Update only injectors 
	\item IBL: Update all injectors and network 
	\item NOT: Do not override 
\end{itemize}
...
DCTL SIM\_MINMAXVOLT CTRL\_Name MAX\_SYNC\_SPEED(pu)
	MIN\_SYNC\_SPEED(pu) DEADTIME(s) Stop\_Simulation(T/F) ;
...
The first line is used to declare the starting of the short-circuit.
The fault has an unknown resistance \emph{(j{*}xfault)} to the ground.
Based on the value of the voltage reached after the fault (declared
in pu), xfault is computed and used for simulating the fault.
...
\section{\index{CHGPRM}Change parameters}
